...
fancyRpartPlot(rpart2Tree$finalModel)
```


```{r RPart Tree Plot}
complexityTree <- fancyRpartPlot(rpartTree$finalModel)
...
library(kernlab)
n <- ncol(train)
tuneVars <- data.frame(vars = seq(2,n,1))

#predfunct function is here as a work around to calling svmBag$fit due to errors being generated from that tune.I found on Stack Exchange that to get the SVM tuning option to work, you can copy/paste the predfunct function that is referenced by svmBag$fit and pass it as its own variable
...
gridRF <- expand.grid(mtry = seq(2,32, by = 2))
...
tree_num <- which(tuneRF$finalModel$forest$ndbigtree == min(tuneRF$finalModel$forest$ndbigtree))

tree_func(final_model = tuneRF$finalModel, tree_num)

tuneRF$finalModel$forest
```
...
#
#
# When using predict, we are able to re-introduce our PH column to the data so we've combined our transformed data and our predictor without the predictor being changed
...
important <- c("mnfFlow","oxyFiller","hydPressure1","bowlSetpoint","fillerLvl", "pressureSetpoint")
featurePlot(imputedData[,important],imputedData[,26])
...
```{r Final Acc Table}
rownames(accTestFull)[5] <- "CIT Bag"
rownames(accTestFull)[2] <- "CART Depth"
accTestFull 
accTestFull2
accTestFull3
...
write_excel_csv(voltron, "C:\\Users\\dcrai\\source\\repos\\DATA624\\Project 2\\Materials\\Summary Table.xlsx")
...
finalPredsSVM <- as.data.frame( #Turn into a dataframe
...
finalPredsRF <- as.data.frame( #Turn into a dataframe
  predict(tuneRF, newdata = imputedRawPH) #Generate predictions using the SVM Model we trained on the 
                                                  # processed prediction data we just processed
  )
nrow(finalPredsRF)

#Write our preds to a file for copy/pasting
write_excel_csv(finalPredsRF, "C:\\Users\\dcrai\\source\\repos\\DATA624\\Project 2\\Materials\\RF Predictions.xlsx")
