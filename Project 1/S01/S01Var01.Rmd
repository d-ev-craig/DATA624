...
DONE5. tsCV - Cross Validate
DONE6. test/train
7. Check best ARIMA models again
  -rotate through all?
  -double check reasons that give clues to the right model from ACF/PACF
  
  
  Could be from outliers
...
#MAPE of .92, which is good if its less than a percentage point, we want something close to 0
...
#Naive & Average
Both the naive and average methods performed pretty poorly with MAPE scores nearly at 100.
```{r}
# Fit the Naive model
...
forecast(naiveModel70, h = 140) %>% forecast::accuracy(tsS01v1)
forecast(naiveModel80, h = 140) %>% forecast::accuracy(tsS01v1)
...
firArima312 <- Arima(tsS01v1, order = c(3,1,2))
fitArima312 #2428.54
...
## Seasonal Stuff Ghetto Checking
```{r}
